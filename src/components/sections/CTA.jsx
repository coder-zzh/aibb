...
const bgUrl = `${import.meta.env.BASE_URL}images/cta-bg.jpg`
...
export default function CTA() {
  return (
    <section className="relative overflow-hidden py-16 md:py-20 text-center">
      ...
      <div className="relative mx-auto mt-14 max-w-3xl border-t border-[var(--border)] px-4 pt-8">
        <p className="text-xs text-[var(--muted-foreground)] mb-4 tracking-widest uppercase">合作伙伴</p>
        <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
          {partners.map(p => (
            <span key={p} className="text-sm font-medium text-[var(--muted-foreground)] opacity-60">
              {p}
            </span>
          ))}
        </div>
